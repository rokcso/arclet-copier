...
export class PopupUI {
  ...
  async initializeTheme() {
    ...
    ThemeService.setupSystemThemeListener(() => {
      const currentTheme = this.settingsUI.elements.appearanceSwitch?.getAttribute("data-value");
    ...
    try {
      ...
      if (tab && tab.url) {
        ...
        if (tab.id && !UrlUtils.isRestrictedPage(tab.url)) {
          ...
          this.currentTitle = tab.title || UrlUtils.generateTitleFromUrl(tab.url) || "特殊页面";
        }
  ...
  async updateUrlDisplay() {
    ...
    const settings = await StorageManager.getUserSettings();
    const processedUrl = UrlProcessor.processUrl(this.currentUrl, settings.urlCleaning);
  ...
  async copyMarkdown() {
    const settings = await StorageManager.getUserSettings();
    const markdownLink = UrlProcessor.createMarkdownLink(this.currentUrl, this.currentTitle, settings.urlCleaning);

    try {
      await ClipboardService.copyFromPopup(markdownLink);
      this.notificationUI.showArcNotification(I18nUtils.getMessage("markdownCopied"));
    } catch (error) {
  ...
  showStatus() {
    ...
    // 显示系统通知
    this.notificationUI.showSystemNotification(EXTENSION_CONFIG.NAME, I18nUtils.getMessage("urlCopied"));
  }
  ...
  bindEventListeners() {
    ...
    if (this.elements.markdownBtn) {
      this.elements.markdownBtn.addEventListener("click", () => this.copyMarkdown());
    }
