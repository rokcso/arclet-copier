# 🏗️ Arclet Copier 构建指南

## 📦 构建系统概述

Arclet Copier 现已集成 **esbuild** 作为构建工具，提供极速的构建体验和代码优化。

### ⚡ 性能对比

| 指标 | 旧方式（文件复制） | esbuild |
|------|------------------|---------|
| **构建速度** | ~1s | ~15ms (66x faster) |
| **代码压缩** | ❌ 无 | ✅ 已启用 |
| **Tree Shaking** | ❌ 无 | ✅ 已启用 |
| **输出大小** | 455KB | 134KB (-70%) |
| **开发模式** | ❌ 无 | ✅ Watch 模式 |
| **Source Map** | ❌ 无 | ✅ 开发模式支持 |

---

## 🚀 快速开始

### 安装依赖

```bash
npm install
```

### 构建命令

#### 1️⃣ 生产构建
```bash
npm run build
```

**输出目录**: `scripts/dist/arclet-copier-v{version}/`

**特性**:
- ✅ 代码压缩和混淆
- ✅ Tree Shaking（移除未使用代码）
- ✅ 生成构建分析报告 (`meta.json`)
- ✅ 显示详细的构建统计信息

**输出示例**:
```
📊 Build Analysis:
   background/background.js: 29.13 KB
   popup/popup.js: 30.78 KB
   options/options.js: 29.24 KB
   batch/batch.js: 32.71 KB
   content/content.js: 11.08 KB
   offscreen/offscreen.js: 0.95 KB

   Total JS: 133.89 KB (0.13 MB)
   Total Package: 644.54 KB (0.63 MB)
```

#### 2️⃣ 开发模式（Watch）
```bash
npm run dev
```

**输出目录**: `dist-dev/`

**特性**:
- ✅ 监听文件变化自动重新构建
- ✅ 内联 Source Map（便于调试）
- ✅ 无代码压缩（构建更快）
- ✅ 实时反馈

**使用场景**:
- 🔧 本地开发和调试
- 🐛 Bug 修复
- ✨ 功能开发

**工作流程**:
1. 运行 `npm run dev`
2. 在 Chrome 中加载 `dist-dev/` 目录
3. 修改代码后自动重新构建
4. 手动刷新扩展（Chrome 扩展页面点击刷新图标）

#### 3️⃣ 旧构建方式（备用）
```bash
npm run build:old
```

保留了原有的简单文件复制构建方式，用于对比或应急使用。

---

## 📋 构建配置详解

### 配置文件
`scripts/esbuild.config.js`

### 核心配置

#### 入口文件
```javascript
entryPoints: [
  "background/background.js",    // Service Worker
  "popup/popup.js",              // 弹窗界面
  "options/options.js",          // 设置页面
  "batch/batch.js",              // 批量操作
  "content/content.js",          // 内容脚本
  "offscreen/offscreen.js",      // 离屏文档
]
```

#### 输出配置
```javascript
bundle: true,           // 打包依赖
format: "esm",          // ES Module 格式
platform: "browser",    // 浏览器平台
target: "chrome96",     // Chrome 96+ (Manifest V3)
minify: isProduction,   // 生产环境压缩
sourcemap: isDev,       // 开发环境 Source Map
treeShaking: true,      // Tree Shaking
```

#### 资源复制
以下文件会自动复制到输出目录：

- **HTML 文件**: `popup/`, `options/`, `batch/`, `offscreen/`
- **CSS 文件**: 所有页面的样式文件
- **静态资源**: `assets/` 目录（图标、图片等）
- **多语言**: `_locales/` 目录
- **第三方库**: `shared/lib/` 目录
- **Manifest**: `manifest.json`

---

## 🔧 开发工作流

### 推荐工作流程

#### 方式 1: Watch 模式（推荐）
```bash
# 终端 1: 启动 watch 模式
npm run dev

# 操作步骤:
# 1. Chrome 访问 chrome://extensions/
# 2. 开启"开发者模式"
# 3. 点击"加载已解压的扩展程序"
# 4. 选择 dist-dev/ 目录
# 5. 修改代码，自动重新构建
# 6. 手动刷新扩展
```

**优点**:
- ⚡ 自动构建，无需手动运行命令
- 🗺️ Source Map 支持，调试方便
- 🔄 快速迭代

**注意**: Chrome 不支持扩展热更新，需要手动刷新扩展。

#### 方式 2: 手动构建
```bash
# 修改代码后
npm run build

# 然后在 Chrome 中刷新扩展
```

### 调试技巧

#### 1️⃣ 使用 Source Map
开发模式下会生成内联 Source Map，可以在 Chrome DevTools 中看到原始源码：

```javascript
// 构建后的代码会包含 source map
//# sourceMappingURL=data:application/json;base64,...
```

#### 2️⃣ 查看构建分析
生产构建后会生成 `meta.json` 文件，包含详细的构建信息：

```bash
# 在输出目录查看
cat scripts/dist/arclet-copier-v1.6.3/meta.json
```

可以使用 [esbuild Bundle Size Analyzer](https://esbuild.github.io/analyze/) 分析该文件。

#### 3️⃣ 日志调试
构建配置包含详细的日志输出：

- ✅ 构建成功/失败状态
- ⚠️ 警告信息
- 📊 文件大小统计
- 📁 输出目录路径

---

## 📦 发布流程

### 1. 生产构建
```bash
npm run build
```

### 2. 测试构建产物
```bash
# 1. 在 Chrome 中加载扩展
# chrome://extensions/ -> 加载已解压的扩展程序
# 选择: scripts/dist/arclet-copier-v{version}/

# 2. 测试所有功能
# - 复制 URL
# - 批量复制
# - 设置修改
# - 多语言切换
# - 短链生成
# - 二维码生成
```

### 3. 打包发布
```bash
cd scripts/dist/
zip -r arclet-copier-v1.6.3.zip arclet-copier-v1.6.3/
```

### 4. 发布到 Chrome Web Store
1. 访问 [Chrome Developer Dashboard](https://chrome.google.com/webstore/devconsole)
2. 上传 ZIP 文件
3. 填写更新日志
4. 提交审核

---

## 🎯 构建优化建议

### 当前优化效果
- ✅ **代码体积减少 70%**: 455KB → 134KB
- ✅ **构建速度提升 66x**: 1s → 15ms
- ✅ **Tree Shaking**: 自动移除未使用代码
- ✅ **代码压缩**: 变量名缩短、空格移除

### 未来优化方向

#### 1️⃣ CSS 优化
```bash
# 考虑引入 CSS 压缩
npm install -D lightningcss
```

当前 CSS 文件未压缩，可节省额外 30-40% 体积。

#### 2️⃣ 图片优化
```bash
# 压缩 assets/ 中的图片
npm install -D imagemin
```

#### 3️⃣ 代码分割
如果未来代码量增长，可以考虑启用代码分割：

```javascript
// esbuild.config.js
splitting: true,  // 启用代码分割
```

---

## 🐛 常见问题

### Q1: 构建失败怎么办？

**A**: 检查以下几点：
1. 确认 Node.js 版本 >= 14
2. 删除 `node_modules/` 重新安装: `npm install`
3. 清理旧的构建产物: `rm -rf dist/ dist-dev/`
4. 查看错误日志中的具体错误信息

### Q2: Watch 模式不工作？

**A**: 可能的原因：
1. 文件权限问题：确保有写入 `dist-dev/` 的权限
2. 端口占用：确保没有其他进程占用
3. 尝试停止后重新运行：`Ctrl+C` 然后 `npm run dev`

### Q3: 扩展加载后不工作？

**A**: 检查清单：
1. 确认加载的是正确的目录（生产构建或开发构建）
2. 查看 Chrome 扩展错误信息
3. 打开 DevTools 查看控制台错误
4. 确认 `manifest.json` 正确复制到输出目录

### Q4: 为什么构建后体积反而变大了？

**A**: 
- 开发模式（`npm run dev`）不压缩代码，体积会更大
- 生产模式（`npm run build`）才会压缩
- 如果生产构建也变大，检查是否引入了新的大型依赖

### Q5: 如何查看构建产物的依赖关系？

**A**: 
```bash
# 生产构建后查看 meta.json
cat scripts/dist/arclet-copier-v{version}/meta.json

# 或使用在线分析工具
# https://esbuild.github.io/analyze/
# 上传 meta.json 文件
```

---

## 📚 相关资源

### 官方文档
- [esbuild 官方文档](https://esbuild.github.io/)
- [Chrome Extension 开发文档](https://developer.chrome.com/docs/extensions/)
- [Manifest V3 迁移指南](https://developer.chrome.com/docs/extensions/mv3/intro/)

### 工具推荐
- [esbuild Bundle Analyzer](https://esbuild.github.io/analyze/) - 构建分析
- [Chrome Extension Source Viewer](https://chromewebstore.google.com/detail/chrome-extension-source-v/jifpbeccnghkjeaalbbjmodiffmgedin) - 查看已安装扩展源码

### 社区
- [Chrome Extensions GitHub](https://github.com/GoogleChrome/chrome-extensions-samples)
- [esbuild GitHub](https://github.com/evanw/esbuild)

---

## 🔄 版本历史

### v1.6.3 (2024-10-10)
- ✨ 引入 esbuild 构建系统
- ⚡ 构建速度提升 66x
- 📦 包体积减少 70%
- 🛠️ 添加开发模式（Watch）
- 📊 添加构建分析报告

---

## 💡 贡献指南

如果你想改进构建配置，请：

1. Fork 项目
2. 创建功能分支：`git checkout -b feature/build-improvement`
3. 修改 `scripts/esbuild.config.js`
4. 测试构建：`npm run build` 和 `npm run dev`
5. 提交 PR

---

## 📝 许可证

GPL-3.0 License - 详见 LICENSE 文件
